...
const App: React.FC = () => {
  ...
  const [darkMode, setDarkMode] = useState<boolean>(() => {
    const saved = localStorage.getItem('iboolimi_dark_mode');
    return saved === 'true';
  });
  ...
  useEffect(() => {
    localStorage.setItem('iboolimi_dark_mode', darkMode.toString());
  }, [darkMode]);
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 ${darkMode ? 'dark bg-gray-950 text-white' : 'bg-[#F9F9F7] text-gray-900'}`}>
      <Navbar 
        language={language} 
      ...
      <main className="flex-grow">
        ...
        {view.type === 'profile' && currentUser && (
          <ProfileView currentUser={currentUser} language={language} darkMode={darkMode} t={t} />
        )}
      </main>

      {showLogin && <LoginModal onLogin={handleLogin} onClose={() => setShowLogin(false)} darkMode={darkMode} t={t} />}
