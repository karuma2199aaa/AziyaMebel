...
const Catalog: React.FC<CatalogProps> = ({ products, categories, language, darkMode, searchQuery, setSearchQuery, onProductClick, t }) => {
    ...
    return (
      <div className="max-w-7xl mx-auto px-6 py-20 animate-in fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {categories.map(cat => (
            <div key={cat.id} className={`group relative aspect-[4/5] overflow-hidden cursor-pointer transition-colors ${darkMode ? 'bg-gray-900' : 'bg-[#F0F0EB]'}`} onClick={() => setSelectedCatId(cat.id)}>
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in">
      <div className="flex flex-col md:flex-row gap-8 mb-16 items-center justify-between">
        <div className="flex items-center space-x-6">
          <button onClick={() => setSelectedCatId(null)} className={`p-3 rounded-full border transition-all ${darkMode ? 'border-gray-700 hover:bg-white hover:text-black' : 'border-gray-200 hover:bg-black hover:text-white'}`}>
        ...
        <input type="text" placeholder={t.search} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full md:w-64 bg-transparent border-b py-2 text-sm outline-none transition-colors ${darkMode ? 'border-gray-700 focus:border-white' : 'border-gray-200 focus:border-black'}`} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-12">
        {filteredProducts.map(product => (
          <div key={product.id} className="group cursor-pointer" onClick={() => onProductClick(product.id)}>
            <div className={`aspect-[3/4] overflow-hidden mb-4 transition-colors ${darkMode ? 'bg-gray-900' : 'bg-[#F0F0EB]'}`}>
