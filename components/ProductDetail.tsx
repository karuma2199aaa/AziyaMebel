...
const ProductDetail: React.FC<ProductDetailProps> = ({ product, allProducts, language, darkMode, onBack, onProductClick, t }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 animate-in fade-in duration-700">
      <button 
        onClick={onBack}
        className={`text-[10px] font-bold tracking-[0.2em] uppercase mb-12 transition-colors flex items-center ${darkMode ? 'text-gray-500 hover:text-white' : 'text-gray-400 hover:text-black'}`}
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 mb-32">
        <div className="lg:col-span-7 space-y-6">
          <div className={`aspect-[4/5] overflow-hidden rounded-sm transition-colors ${darkMode ? 'bg-gray-900' : 'bg-[#F0F0EB]'}`}>
            <img 
          ...
          <div className="grid grid-cols-4 gap-4">
            {product.images.map((img, idx) => (
              <button 
                ...
                className={`aspect-square overflow-hidden border transition-all ${activeImage === idx ? (darkMode ? 'border-white' : 'border-black') : 'border-transparent opacity-60'}`}
              >
        ...
        <div className="lg:col-span-5 space-y-12">
          ...
          {!orderSuccess ? (
            <form onSubmit={handleOrder} className={`p-8 space-y-6 mt-12 border rounded-2xl shadow-sm transition-colors ${darkMode ? 'bg-gray-900 border-gray-800' : 'bg-[#F9F9F7] border-[#E5E5E0]'}`}>
              ...
              <button 
                ...
                className={`w-full py-5 text-[10px] font-bold tracking-[0.3em] uppercase transition-colors disabled:opacity-50 rounded-xl ${darkMode ? 'bg-white text-black hover:bg-gray-200' : 'bg-black text-white hover:bg-gray-800'}`}
              >
            ...
            <div className={`p-12 text-center space-y-6 animate-in slide-in-from-top-4 border rounded-3xl transition-colors ${darkMode ? 'bg-gray-900 border-green-900/30' : 'bg-[#F1F5E8] border-[#8E9775]/20'}`}>
      ...
      {similarProducts.length > 0 && (
        <div className={`mt-20 border-t pt-20 transition-colors ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {similarProducts.map(item => (
              <div key={item.id} className="group cursor-pointer" onClick={() => onProductClick(item.id)}>
                <div className={`aspect-[3/4] overflow-hidden mb-4 rounded-sm transition-colors ${darkMode ? 'bg-gray-900' : 'bg-[#F0F0EB]'}`}>
